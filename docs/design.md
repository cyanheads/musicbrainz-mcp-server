...
| `musicbrainz_get_release_group` | Release-group ("the album" above specific pressings) by MBID: primary + secondary type, first-release date, artist credit, the list of releases (editions), tags/genres, and cover-art availability from the Cover Art Archive (WS/2 has no release-group stub). Use `musicbrainz_get_release` for a specific edition's tracklist. | `mbid` (UUID) | `readOnlyHint`, `openWorldHint` |
...
| `musicbrainz_get_cover_art` | Cover Art Archive images for a release or release-group MBID: front/back flags, image types, full-resolution URLs, and 250/500/1200px thumbnails. Returns an empty image set (not an error) when the entity has no art; an MBID the archive has no record of is checked against MusicBrainz first, so a malformed or nonexistent one fails instead. Separate from `musicbrainz_get_release` so art is fetchable without the full record. | `mbid` (UUID), `entity_type` (enum: release, release-group; default release) | `readOnlyHint`, `openWorldHint` |
...
| `cover-art-service` | Cover Art Archive (`coverartarchive.org`) — release / release-group image metadata + thumbnail/full URLs. Treats 404 as "no art" (empty set flagged `found: false`), not an error. | `get_cover_art`, and `get_release_group` for its availability stub (WS/2 carries no release-group stub). `get_release`'s stub still comes free from the WS/2 release payload — no CAA call. |
...
3. **`cover-art-service`** — `getImages(entityType, mbid)`; 404 → empty set flagged `found: false`.
...
The split matters: ISRC/ISWC are true endpoints (exact, deterministic), barcode is a Lucene search filter (ranked, may return near-matches). Because the barcode value is interpolated into that filter, the handler accepts digits only — spaces and hyphens (how barcodes are printed) are removed first, anything else is `invalid_identifier` before the call, so a wildcard cannot widen the search into a broad release listing. The handler routes by `id_type`; the output schema is a discriminated union on which entity type came back (recordings | works | releases), so the agent knows what it's holding.
...
- **A release-group's availability comes from CAA, not WS/2.** Live verification: `GET /ws/2/release-group/{mbid}` carries no `cover-art-archive` key at all, whatever `inc` is requested — only releases get the stub. Reporting `exists: false` from a missing stub made `get_release_group` contradict `get_cover_art` for the same MBID, so the tool now resolves availability through the CAA release-group lookup, started before the WS/2 lookup and awaited after it (different host, outside the MusicBrainz limiter, so it overlaps rather than serializing). It is best-effort and bounded — one attempt, short timeout: a CAA failure omits `coverArt` (optional on this tool only) and says so in the notice, because a completed lookup is the only thing that licenses reporting `false`. Cost, measured live: a release-group *with* art goes from ~170 ms to ~1.0 s cold (the CAA release-group endpoint redirects twice, via archive.org); one *without* art and every cached call are unchanged.

- **A CAA 404 is verified against WS/2 before `get_cover_art` reports "no art".** Live: the archive answers 404 identically for a real entity with no art, the all-zeros sentinel, and a well-formed MBID matching nothing — it reserves 400 for a non-UUID. A WS/2 lookup of the same entity type splits them (400 / 404 / 200), so `get_cover_art` makes that one bounded call on a 404 and maps it to `invalid_mbid` / `entity_not_found` / the genuine no-art success. It stays in the tool, not in `getImages()`, so other callers pay nothing; and it is best-effort — a WS/2 5xx or timeout returns the no-art success with the notice downgraded to "existence could not be confirmed" rather than turning an informative empty set into an error.
...
Per-tool typed contracts (`errors: [{ reason, code, when, recovery, retryable? }]`). Baseline codes (`ServiceUnavailable`, `Timeout`, `ValidationError`, `SerializationError`, `InternalError`) bubble freely; a domain failure that lands on one of them is declared anyway — `ValidationError` carries a bad MBID, identifier, or query — so the caller gets a `reason` and a recovery hint instead of a bare code. The live API exposes a sharp **400-vs-404 split** that the contracts encode:

| Tool(s) | reason | code | when | retryable |
|:--------|:-------|:-----|:-----|:----------|
| all `get_*` | `invalid_mbid` | `ValidationError` | MBID is malformed or the all-zeros sentinel — MusicBrainz returns **HTTP 400** `{"error":"Invalid mbid."}` | no |
...
| `get_cover_art` | `invalid_mbid` | `ValidationError` | MBID is not a UUID (CAA returns **400**), or is the all-zeros sentinel (CAA returns 404 and the WS/2 check returns 400) | no |
| `get_cover_art` | `entity_not_found` | `NotFound` | CAA returns 404 and the WS/2 check of the same entity type returns 404 — no such release / release-group | no |
| `get_cover_art` | *(none — empty set)* | — | No art for a verified entity — CAA 404 + WS/2 200, mapped to an empty image set with a notice, **not** an error. A WS/2 check that itself fails keeps the empty set and says existence is unconfirmed | — |
| `lookup_identifier` | `identifier_not_found` | `NotFound` | Valid-format ISRC/ISWC/barcode that resolves to zero entities | no |
| `lookup_identifier` | `invalid_identifier` | `ValidationError` | Malformed ISRC/ISWC (the dedicated endpoint returns **400**), or a barcode holding anything but digits once spaces/hyphens are removed (rejected locally, before the call) | no |
| `search_entities` | `blank_query` | `ValidationError` | Query is whitespace-only — rejected locally, before the call, instead of MusicBrainz's generic blank-query **400** | no |
| `browse_entities` | `invalid_link` | `ValidationError` | Not exactly one link MBID, or a malformed one (**HTTP 400**) | no |
| `browse_entities` | `entity_not_found` | `NotFound` | The link MBID is well-formed but no such parent exists (**HTTP 404**) | no |
...
The empty-result cases (`get_cover_art` no-art, search/browse zero-hits) deliberately return success with an `enrichment` notice rather than throwing — an empty set is information, and forcing the agent to catch an error for "no art exists" would be a worse interface. That only holds for an entity the server knows exists, which is why a CAA 404 is checked against WS/2 first.
...
- **Cover Art Archive:** `https://coverartarchive.org/{release|release-group}/{mbid}` → `{ images: [ { id, front, back, types[], approved, image, thumbnails: { 250, 500, 1200, small, large } } ], release }`. **404 when no art** (HTML body, not JSON — treated as empty, not an error) — and the same 404 for the all-zeros sentinel and for a well-formed MBID matching no entity; **400** only for a non-UUID. The MBID-existence split therefore has to come from WS/2, which does return 400 vs 404 for those two. Art is served at the *release* level; a release-group MBID returns **HTTP 307** redirecting to `archive.org/download/mbid-{release-mbid}/index.json`, which resolves to the same `{ images[], release }` shape — `release` identifies the representative release chosen. The `cover-art-service` must follow redirects; the response shape is identical after the redirect.
